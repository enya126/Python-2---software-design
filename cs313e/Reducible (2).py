...
def insert_word(s, hash_table):
    ...
    if hash_table[idx] == "":
        ...
        stepSize = 1
        # add to the hash table of an empty position
        while (hash_table[(idx + newPos * stepSize) % len(hash_table)] != ""):
            stepSize += 1
        hash_table[(idx + newPos * stepSize) % len(hash_table)] = s
...
def main():
  ...
  m=0
  for m in range(int(length*0.2), int(length*2)):
      if is_prime(m):
          break
